...
.."\n#Silent Contacts: "..(redis:get("mute:contact"..target) or 'no').."" 
.."\n#Silent Photo: "..(redis:get("mute:photo"..target) or "no")..""
--.."\nMute Video: "..(redis:get("mute:video"..target) or 'no')..""
.."\n#Silent Text: "..(redis:get("mute:text"..target) or 'no')..""
.."\n#Silent Gifs: "..(redis:get("mute:gif"..target) or 'no')..""
.."\n#Silent Audio: "..(redis:get("mute:audio"..target) or 'no').."" 
.."\n#Silent All: "..(redis:get("mute:all"..target) or "no")
  ...
  vardump(result)
    if result.to.peer_type == "channel" then
		...
		if is_mute(result.from.peer_id, result.to.peer_id) then
			send_large_msg(channel, name.." ["..result.from.peer_id.."] alowed to chat.")
			redis:srem(hash, result.from.peer_id)
		else
			send_large_msg(channel, name.." ["..result.from.peer_id.."] not alowed to chat.")
...
local function run(msg, matches)
  ...
  if matches[1] == "silent" and is_momod(msg) then
    if matches[2]:lower() == "audio" then
      ...
      return "Silent Audio has been enabled"
    end
    if matches[2]:lower(0) == "text" then
      redis:set("mute:text"..target, "yes")
      return "Silent Text has been enabled"
    end
	...
	if matches[2]:lower() == "gifs" then
	  ...
	  return "Gifs sharing has been enabled"
	end
	if matches[2]:lower() == "contacts" then
	  redis:set("mute:contact"..target, "yes")
	  return "Share contact has been else"
	end
  ...
  if matches[1] == "unsilent" and is_momod(msg) then
    if matches[2]:lower() == "audio" then
      ...
      return "Silent Audio has been disabled"
    end
    if matches[2]:lower() == "text" then
      redis:del("mute:text"..msg.to.id)
      return "Silent Text has been disabled"
    end
    if matches[2]:lower() == "photo" then
