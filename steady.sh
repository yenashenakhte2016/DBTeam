...
echo -e "$bld$f4...در حال بررسی$rst" # Time between checking cpu calls of the cli process. Set the value high if your bo
sleep 0.5
ls ../ | grep $BOT 2>/dev/null >/dev/null
if [ $? != 0 ]; then
  echo -e "$f1 متاسفانه $BOT در دایرکتوری شما یافت نشد$rst" # Time between checking cpu calls of the cli process. Set the value high if your bo
  sleep 4
  exit 1
fi
echo -e "$f2 $BOT در دایرکتوری یافت شد$rst"
sleep 0.5


echo ""
echo -e "\033[38;5;208m By @MohamamdDeveloper\033[0;00m"
echo -e "\033[38;5;208m MuteTeam\033[0;00m"
echo -e "\033[38;5;208m Waiting...\033[0;00m"
echo -e "\033[38;5;208m \033[0;00m"
echo -e "\033[38;5;208m \033[0;00m"

sleep 1.5
echo -e "$bld$f4 ...صبر کنید$rst"
sleep 0.7

# Looks for the number of screen/telegram-cli processes
CLINUM=`ps -e | grep -c telegram-cli`
echo "$f2 اجرا $CLINUM برای کیل تلگرام سی ال ای$rst"
sleep 0.9

# =====Setup ends===== #

# Opening new tmux in a daemon
echo -e "$bld$f4 ...بررسی نهایی$rst"
...
echo -e "$f2 تعداد برنامه های تلگرام سی ال ای: $CLIPID$rst"
echo ""
echo ""
...
while true; do
        ...
        echo -e "$f2 تعداد دفعات ریلانچ شده:$f5 $OK $rst"
        echo -e "$f2 تعداد دفعات رفع کرش:$f5 $BAD $rst"
        echo ""
        ...
        if [ $NONVOLUNTARY != $NONVOLUNTARYCHECK ] || [ $VOLUNTARY != $VOLUNTARYCHECK ]; then
                ...
                if [ -z "${CLIPID}" ]; then
                        echo -e "$f1 ...خطا: تلگرام سی ال ای اجرا نشد$rst"
                        echo -e "$f1 ...خطا: در ریلانچ ربات$rst"
                        sleep 3
                        exit 1
                fi
        ...
        # Reload steady.sh after 1h
        if [ "$OK" == 70 ]; then
                sync
                sudo sh -c echo '"tmux new-session -d -s script "bash steady.sh -t"'
        fi
...
# Checking if the bot folder is in HOME
echo -e "$bld$f4 ...چک کردن دایرکتوری$rst"
sleep 0.5
ls ../ | grep $BOT 2>/dev/null >/dev/null
if [ $? != 0 ]; then
  echo -e "$f1 خطا $BOT ...در دایرکتوری یافت نشد$rst"
  sleep 4
  exit 1
fi
echo -e "$f2 $BOT .دایرکتوری یافت شد$rst"
sleep 0.5


echo ""
echo -e "\033[38;5;208m By @MohammadDeveloper\033[0;00m"
echo -e "\033[38;5;208m MuteTeam\033[0;00m"
echo -e "\033[38;5;208m Waiting...\033[0;00m"
echo -e "\033[38;5;208m \033[0;00m"
echo -e "\033[38;5;208m \033[0;00m"
...
sleep 1.5
echo -e "$bld$f4 ...چک کردن برنامه$rst"
sleep 0.7
  ...
  case $opt in
    ...
    T)
      ...
      ;;
        ...
        echo -e "\e[1m"
        echo -e ""
        echo "steady.sh bash script v1.2 iicc 2016 NOD32" >&2
        echo ""
        echo -e "\e[0m"
